...
import { list } from './firestore.service';
...
export const getStudentAccountByStudentID = async (studentAccountID: string, collectionName: string) => {
  const dataList = await list(collectionName);
  return dataList.filter((account) => account.id == studentAccountID)[0];
};
